...
use cs_domain::{CalendarSpread, CalendarSpreadResult, EarningsEvent, CONTRACT_MULTIPLIER};
    ...
    fn validate_entry(
        ...
        if pricing.net_cost < config.min_entry_cost {
            return Err(ExecutionError::InvalidSpread(format!(
                ...
                pricing.net_cost, config.min_entry_cost, short_leg.price, long_leg.price,
            )));
        }
