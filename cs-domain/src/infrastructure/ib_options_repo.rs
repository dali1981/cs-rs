...
use ib_data_collector::domain::{BarType, Contract, OptionType as IbOptionType};
...
impl IbOptionsRepository {
    ...
    fn group_contracts(
        &self,
        symbol: &str,
    ) -> Result<HashMap<(NaiveDate, Decimal, IbOptionType), Vec<Contract>>, RepositoryError> {
        let contracts = self.db.get_symbol_contracts(symbol).map_err(|e| {
            RepositoryError::NotFound(format!("Failed to get contracts for {}: {}", symbol, e))
        })?;

        let mut grouped: HashMap<(NaiveDate, Decimal, IbOptionType), Vec<Contract>> =
            HashMap::new();
    ...
    fn find_latest_bar(
        bars: &[ib_data_collector::database::repository::Bar],
        target_time: DateTime<Utc>,
    ) -> Option<&ib_data_collector::database::repository::Bar> {
        bars.iter()
...
impl OptionsDataRepository for IbOptionsRepository {
    ...
    async fn get_option_bars_at_time(
        ...
        for ((expiration, strike, option_type), contracts) in grouped {
            ...
            if let (Some(bid_c), Some(ask_c)) = (bid_contract, ask_contract) {
                // Read bars
                let bid_bars = self.db.read_bars(bid_c).map_err(|e| {
                    RepositoryError::NotFound(format!("Failed to read bid bars: {}", e))
                })?;
                let ask_bars = self.db.read_bars(ask_c).map_err(|e| {
                    RepositoryError::NotFound(format!("Failed to read ask bars: {}", e))
                })?;
    ...
    async fn get_available_expirations(
        ...
        let contracts = self.db.get_symbol_contracts(underlying).map_err(|e| {
            RepositoryError::NotFound(format!("Failed to get contracts for {}: {}", underlying, e))
        })?;
