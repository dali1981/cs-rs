...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Searching for VWAV earnings between {} and {}\n",
        start, end
    );
    ...
    let tv_events = tv_repo
        .load_earnings(start, end, Some(&["VWAV".to_string()]))
        .await?;
    ...
    // Check Nasdaq source
    let nasdaq_repo =
        EarningsReaderAdapter::with_source(earnings_dir.clone(), earnings_rs::DataSource::Nasdaq);
    let nasdaq_events = nasdaq_repo
        .load_earnings(start, end, Some(&["VWAV".to_string()]))
        .await?;
    ...
    // Check Yahoo source
    let yahoo_repo =
        EarningsReaderAdapter::with_source(earnings_dir, earnings_rs::DataSource::Yahoo);
    let yahoo_events = yahoo_repo
        .load_earnings(start, end, Some(&["VWAV".to_string()]))
        .await?;
